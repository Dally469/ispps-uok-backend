...
from app.utils.access import attendance_visibility_clause, auth_user_id, resolve_school_scope
...
async def create_attendance(
    ...
    if auth.get("role") == "teacher":
        if not enrollment.course or enrollment.course.teacher_id != auth_user_id(auth):
...
async def bulk_attendance(
    ...
    if auth.get("role") == "teacher" and course.teacher_id != auth_user_id(auth):
        raise HTTPException(status_code=403, detail="You can only manage attendance for your courses")
