...
from app.models import Attendance, Course, Enrollment, ParentLink, Prediction, Student
...
def student_visibility_clause(auth: dict):
    ...
    if role == "teacher":
        clause = Student.enrollments.any(Enrollment.course.has(Course.teacher_id == user_id))
    ...
    if role == "parent":
        clause = Student.parent_links.any(ParentLink.parent_user_id == user_id)
        return and_(Student.school_id == school_id, clause) if school_id else clause
...
def course_visibility_clause(auth: dict):
    ...
    if role == "teacher":
        clause = Course.teacher_id == user_id
    ...
    if role == "parent":
        clause = Course.enrollments.any(
            Enrollment.student.has(
                Student.parent_links.any(ParentLink.parent_user_id == user_id)
            )
        )
        return and_(Course.school_id == school_id, clause) if school_id else clause
...
def attendance_visibility_clause(auth: dict):
    ...
    if role == "teacher":
        return Attendance.enrollment.has(Enrollment.course.has(Course.teacher_id == user_id))

    if role == "student":
        return Attendance.enrollment.has(Enrollment.student.has(Student.user_id == user_id))

    if role == "parent":
        return Attendance.enrollment.has(
            Enrollment.student.has(
                Student.parent_links.any(ParentLink.parent_user_id == user_id)
            )
        )
...
def prediction_visibility_clause(auth: dict):
    ...
    if role == "teacher":
        direct_course_clause = Prediction.course.has(Course.teacher_id == user_id)
        overall_student_clause = and_(
            Prediction.course_id.is_(None),
            Prediction.student.has(
                Student.enrollments.any(Enrollment.course.has(Course.teacher_id == user_id))
            ),
        )
...
def filter_enrollments_for_role(enrollments: list[Enrollment] | None, auth: dict) -> list[Enrollment]:
    ...
    user_id = auth_user_id(auth)
    return [enrollment for enrollment in items if enrollment.course and enrollment.course.teacher_id == user_id]